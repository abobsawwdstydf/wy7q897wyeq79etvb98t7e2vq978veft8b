...
import { prisma, isDbHealthy, checkDbHealth } from './db';
...
app.use(cors({
  origin: (origin, callback) => {
    if (!origin && config.isProduction) return callback(null, false);
...
const authLimiter = rateLimit({
  ...
  legacyHeaders: false,
  skip: (req) => {
    return req.path === '/me' || req.path === '/check-username';
  },
});
...
// File serving routes (require auth)
...
app.use('/api/profile-music', apiLimiter, authenticateToken, profileMusicRoutes);
...
app.use('/api/nft', apiLimiter, authenticateToken, nftRoutes);
app.use('/api/wall', apiLimiter, authenticateToken, wallRoutes);
...
app.use('/api/badges', apiLimiter, authenticateToken, badgesRoutes);
app.use('/api/playlists', apiLimiter, authenticateToken, playlistsRoutes);
app.use('/api/drawing-chat', apiLimiter, authenticateToken, drawingChatRoutes);
app.use('/api/map-routes', apiLimiter, authenticateToken, mapRoutesRoutes);
app.use('/api/fake-password', apiLimiter, authenticateToken, fakePasswordRoutes);
...
// Bot API (uses bot token auth, not user auth)
...
// DB health check every 2 minutes (keeps Neon awake + detects when it's back)
setInterval(checkDbHealth, 2 * 60 * 1000);
checkDbHealth();
