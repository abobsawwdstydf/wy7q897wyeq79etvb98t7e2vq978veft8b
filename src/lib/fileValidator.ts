...
export async function validateFileType(buffer: Buffer, declaredMimeType: string): Promise<{
  ...
  try {
    ...
    if (!fileType) {
      // SECURITY FIX: Reject files with unrecognized magic bytes (prevents crafted malicious files)
      // Only allow text/plain and application/octet-stream as fallback
      const safeFallback = ['text/plain', 'application/octet-stream'].includes(declaredMimeType.split(';')[0].trim().toLowerCase());
      if (safeFallback) {
        return { valid: true };
      }
      return { valid: false, error: 'Файл не распознан. Загрузите файл в поддерживаемом формате.' };
    }
