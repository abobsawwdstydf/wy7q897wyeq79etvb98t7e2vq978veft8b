...
// Rate limiter for device auth to prevent brute-force
...
router.post('/register', loginLimiter, upload.single('avatar') as any, async (req: Request, res: Response) => {
  try {
    ...
    if (!username || !displayName || !password) {
      res.status(400).json({ error: 'Username, имя и пароль обязательны' });
      return;
    }
...
router.post('/device/init', deviceAuthLimiter, async (req: Request, res: Response) => {
  try {
    const { token } = req.body;
    if (!token || typeof token !== 'string' || token.length < 32 || token.length > 128 || !/^[a-zA-Z0-9_-]+$/.test(token)) {
...
router.post('/device/scan', deviceAuthLimiter, async (req: Request, res: Response) => {
  try {
    const { device } = req.body;
    if (!device || typeof device !== 'string' || device.length < 32 || device.length > 128 || !/^[a-zA-Z0-9_-]+$/.test(device)) {
...
router.get('/device/check', deviceAuthLimiter, async (req: Request, res: Response) => {
  try {
