...
router.get('/global', async (req: AuthRequest, res) => {
  try {
    ...
    try {
      // SECURITY FIX: Use Prisma.sql tagged templates to prevent SQL injection
      const chatIdParams = Prisma.join(chatIds.map(id => id));
      ...
      // Search via FTS5 — safe from SQL injection
      const ftsResults = await prisma.$queryRaw<any[]>(
        Prisma.sql`SELECT m.*, m.rowid as rowid
         FROM "MessageFTS" fts
         JOIN "Message" m ON m.rowid = fts.rowid
         WHERE "MessageFTS" MATCH ${ftsQuery}
         AND m."chatId" IN (${chatIdParams})
