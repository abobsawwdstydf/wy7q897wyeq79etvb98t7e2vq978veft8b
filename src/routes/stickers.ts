...
/**
 * Get all public sticker packs (paginated)
 */
router.get('/packs', async (req: AuthRequest, res) => {
  try {
    ...
    res.json({ packs, total, hasMore: offset + limit < total });
  } catch (error) {
...
/**
 * Get stickers from a pack (paginated)
 */
router.get('/packs/:packId/stickers', async (req: AuthRequest, res) => {
  try {
    ...
    const [stickers, total] = await Promise.all([
      ...
      prisma.sticker.count({ where: { packId } })
    ]);

    res.json({ stickers, total, hasMore: offset + limit < total });
  } catch (error) {
