  ...
  socket.on('send_message', async (data: {
    ...
    try {
      ...
      if (data.mediaUrl) {
        ...
        const isSticker = data.mediaUrl.startsWith('/stickers/');
        const isAnimatedEmoji = data.mediaUrl.startsWith('/animated-emojis/');

        if (!isExternalUrl && !isLocalUpload && !isApiFile && !isSticker && !isAnimatedEmoji) {
          socket.emit('error', { message: 'Недопустимый mediaUrl' });
          return;
        }
        if ((isLocalUpload || isSticker || isAnimatedEmoji) && data.mediaUrl.includes('..')) {
