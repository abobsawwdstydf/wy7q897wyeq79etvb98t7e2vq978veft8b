...
import { isTokenRevoked } from '../services/auth';
...
export function setupSocket(io: Server) {
  ...
  io.use(async (socket: AuthSocket, next) => {
    ...
    try {
      const decoded = jwt.verify(token, config.jwtSecret) as { userId: string; type?: string; jti?: string };
      // SECURITY: Verify token type is 'access' (not refresh token)
      if (decoded.type && decoded.type !== 'access') {
        return next(new Error('Недействительный тип токена'));
      }
      // SECURITY: Check if token has been revoked (logout, password change, etc.)
      if (decoded.jti && await isTokenRevoked(decoded.jti)) {
        return next(new Error('Токен отозван'));
      }
