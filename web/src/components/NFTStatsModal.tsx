...
export default function NFTStatsModal({ itemId, itemType, onClose }: NFTStatsModalProps) {
  ...
  const priceChange = item.currentPrice - item.priceFromНексо;
  const priceChangePercent = item.priceFromНексо > 0 ? ((priceChange / item.priceFromНексо) * 100).toFixed(2) : '0';
  const isPositive = priceChange >= 0;

  const soldOut = item.currentSupply >= item.totalSupply;
  const soldPercent = ((item.currentSupply / item.totalSupply) * 100).toFixed(1);
