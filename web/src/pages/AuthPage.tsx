...
const InputField = ({
  gifSrc, type = 'text', value, onChange, placeholder, autoFocus, autoComplete, inputMode,
  className = '', right, disabled, onKeyDown
}: {
  gifSrc?: string; type?: string; value: string; onChange: (e: React.ChangeEvent<HTMLInputElement>) => void;
    ...
    <div className="relative">
      {gifSrc && (
        <div className="absolute left-3 top-1/2 -translate-y-1/2 z-10">
          <AuthGifIcon src={gifSrc} size={20} focused={focused} />
        </div>
      )}
      <input
        ...
        className={`w-full ${gifSrc ? 'pl-11' : 'pl-4'} ${right ? 'pr-11' : 'pr-4'} py-3.5 rounded-xl bg-white/[0.04] border border-white/[0.08] text-white placeholder-white/25 focus:border-[#6366f1]/50 focus:bg-white/[0.06] focus:ring-2 focus:ring-[#6366f1]/20 transition-all duration-300 outline-none disabled:opacity-40 text-[15px] ${className}`}
      />
...
type Mode = 'landing' | 'register' | 'register-done';
...
export default function AuthPage() {
  ...
  const stepOk = useCallback((): boolean => {
    switch (step) {
      ...
      case 2: return password.length >= 6 && password.length <= 50 && password === confirmPassword;
      default: return false;
    }
  }, [step, username, usernameStatus, displayName, password, confirmPassword]);
  ...
  const nextStep = () => { setError(''); if (validate() && step < 2) setStep(step + 1); };
  const prevStep = () => { setError(''); if (step > 1) setStep(step - 1); };

  const handleLogin = async () => {
    setError('');
    if (username.length < 3) { setError('Минимум 3 символа'); return; }
    if (loginPassword.length < 6) { setError('Пароль минимум 6 символов'); return; }
    setSubmitting(true);
    try { await login(username, loginPassword); }
  ...
  const handleRegister = async () => {
    ...
    try {
      await register({
        ...
        bio: bio || undefined, avatar: avatarFile || undefined,
      });
            ...
            <div className="flex items-center justify-center gap-1.5 mb-4">
              {[1, 2].map(n => (
                <div key={n} className="flex items-center gap-1.5">
                  ...
                  {n < 2 && <div className={`w-4 h-[2px] rounded-full ${n < step ? 'bg-[#6366f1]/50' : 'bg-white/[0.08]'}`} />}
                </div>
              ))}
            </div>

            <AnimatePresence mode="wait">
              <motion.div key={step} initial={{ opacity: 0, x: 30, scale: 0.97 }} animate={{ opacity: 1, x: 0, scale: 1 }} exit={{ opacity: 0, x: -30, scale: 0.97 }} transition={{ type: 'spring', stiffness: 300, damping: 25 }}>

                {step === 1 && (
                  <div className="space-y-3">
                    <label className="block text-[12px] font-medium text-white/50 ml-1 flex items-center gap-2"><span className="text-[14px]">@</span> Username <span className="text-red-400">*</span></label>
                    <input type="text" value={username} onChange={e => { const v = e.target.value.replace(/[^a-zA-Z0-9_.-]/g, '').slice(0, 17); setUsername(v); playKeyboardSound(); }}
                      ...
                      className={`w-full px-4 py-3.5 rounded-xl bg-white/[0.04] border text-white placeholder-white/25 focus:ring-2 transition-all duration-300 outline-none ${usernameStatus === 'taken' ? 'border-red-500/50' : usernameStatus === 'available' ? 'border-emerald-500/50' : 'border-white/[0.08]'} focus:border-[#6366f1]/50 focus:ring-[#6366f1]/20`} />
                ...
                {step === 2 && (
                  <div className="space-y-4">
                    <div className="space-y-3">
                      <label className="block text-[12px] font-medium text-white/50 ml-1 flex items-center gap-2"><span className="text-[14px]">🔑</span> Пароль <span className="text-red-400">*</span></label>
                      <div className="relative">
                        <div className="absolute left-3 top-1/2 -translate-y-1/2 z-10">
                        ...
                        <input type={showPw ? 'text' : 'password'} value={password} onChange={e => { setPassword(e.target.value); playKeyboardSound(); }}
                          ...
                          className="w-full pl-11 pr-11 py-3.5 rounded-xl bg-white/[0.04] border border-white/[0.08] text-white placeholder-white/25 focus:border-[#6366f1]/50 focus:ring-2 focus:ring-[#6366f1]/20 transition-all duration-300 outline-none" />
                      ...
                      <div className="relative">
                        <input type={showPw2 ? 'text' : 'password'} value={confirmPassword} onChange={e => { setConfirmPassword(e.target.value); playKeyboardSound(); }}
                          ...
                          className="w-full px-4 py-3.5 rounded-xl bg-white/[0.04] border border-white/[0.08] text-white placeholder-white/25 focus:border-[#6366f1]/50 focus:ring-2 focus:ring-[#6366f1]/20 transition-all duration-300 pr-11 outline-none" />
                    ...
                    <div>
                      <label className="block text-[12px] font-medium text-white/50 mb-1.5 ml-1 flex items-center gap-2"><Shield size={14} /> Облачный пароль</label>
                      <div className="relative">
                        <div className="absolute left-3 top-1/2 -translate-y-1/2 z-10">
                          <AuthGifIcon src="/auth-gifs/lock.gif" size={18} />
                        </div>
                        <input type="text" value={cloudPassword} onChange={e => { setCloudPassword(e.target.value); playKeyboardSound(); }}
                          placeholder="Необязательно" className="w-full pl-10 pr-4 py-3 rounded-xl bg-white/[0.04] border border-white/[0.08] text-white placeholder-white/25 focus:border-[#6366f1]/50 focus:ring-2 focus:ring-[#6366f1]/20 transition-all duration-300 outline-none" />
                      </div>
            ...
            <div className="flex gap-2.5 mt-5">
              ...
              {step < total ? (
                <motion.button onClick={nextStep}
                  whileHover={{ scale: 1.01, boxShadow: '0 0 25px rgba(99,102,241,0.5)' }}
                  whileTap={{ scale: 0.98 }}
                  className="flex-1 py-3.5 rounded-xl font-semibold text-[14px] flex items-center justify-center gap-2 min-h-[48px] text-white relative overflow-hidden"
                ...
                <motion.button onClick={handleRegister} disabled={submitting || !stepOk()}
                  whileHover={{ scale: 1.01, boxShadow: '0 0 25px rgba(99,102,241,0.5)' }}
                  whileTap={{ scale: 0.98 }}
                  className="flex-1 py-3.5 rounded-xl font-semibold text-[14px] flex items-center justify-center gap-2 disabled:opacity-50 min-h-[48px] text-white relative overflow-hidden"
                  style={{ background: 'linear-gradient(135deg, #6366f1, #8b5cf6)', boxShadow: '0 0 15px rgba(99,102,241,0.3)' }}>
                  <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/10 to-transparent -translate-x-full hover:translate-x-full transition-transform duration-700" />
      ...
      <div className="relative z-10 w-full max-w-sm mx-4">
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: [0.22, 1, 0.36, 1] }}
          ...
          <div className="relative mb-4 inline-block">
            ...
            <motion.img
              src="/auth-gifs/mail.gif" alt=""
              className="absolute -bottom-2 -left-5 w-7 h-7 drop-shadow-lg"
              animate={{ y: [0, -5, 0], rotate: [0, -8, 8, 0] }}
              transition={{ duration: 3.5, repeat: Infinity, ease: 'easeInOut', delay: 0.5 }}
              initial={{ opacity: 0, scale: 0 }}
            />
            <motion.img
              src="/auth-gifs/lock.gif" alt=""
              className="absolute -top-2 -left-4 w-7 h-7 drop-shadow-lg"
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4, duration: 0.5 }}>
          <div style={GLASS} className="rounded-2xl p-5 relative overflow-hidden">
            ...
            <div className="space-y-3">
              <InputField type="text" value={username} onChange={e => { const v = e.target.value.replace(/[^a-zA-Z0-9_.-]/g, '').slice(0, 17); setUsername(v); }} placeholder="Username" autoFocus autoComplete="username" onKeyDown={e => e.key === 'Enter' && (loginPassword ? handleLogin() : undefined)} />
              <InputField gifSrc="/auth-gifs/lock.gif" type={loginShowPw ? 'text' : 'password'} value={loginPassword} onChange={e => setLoginPassword(e.target.value)} placeholder="Пароль" autoComplete="current-password" onKeyDown={e => e.key === 'Enter' && handleLogin()}
              ...
              <motion.button onClick={handleLogin} disabled={submitting}
                whileHover={{ scale: 1.01, boxShadow: '0 0 30px rgba(99,102,241,0.5), 0 8px 24px rgba(0,0,0,0.3)' }}
                whileTap={{ scale: 0.98 }}
                className="w-full py-3.5 rounded-xl font-semibold text-[14px] flex items-center justify-center gap-2 transition-all disabled:opacity-50 text-white relative overflow-hidden"
            ...
            <div className="mt-4 pt-3 border-t border-white/[0.06] flex items-center justify-center">
